<script>
  ...
  let manualT = 0, lastTs = 0, duration = 7.8, isPlaying = false, speed = 1;
  const W = 1080, H = 1920;
  ...
  function buildTimeline() {
    ...
    tl.set('.kamusai-mini-flow', { display: 'flex' }, 3.2);
  ...
  <div class="kamusai-sec kamusai-llm">
    <div class="kamusai-llm-container">
      ...
      <div class="kamusai-llm-diagram">
        <div class="kamusai-llm-input-box"><span class="icon-emoji">⌨️</span><br><span class="label-text">TEKS</span></div>
        <div class="kamusai-llm-arrow-right">→</div>
        <div class="kamusai-llm-model-box"><span class="icon-emoji">🤖</span></div>
        <div class="kamusai-llm-arrow-left">→</div>
        <div class="kamusai-llm-output-box"><span class="icon-emoji">📄</span><br><span class="label-text">TEKS</span></div>
      </div>
...
.kamusai-llm-input-box,.kamusai-llm-model-box,.kamusai-llm-output-box{background:rgba(0,191,255,0.1);border:2px solid rgba(0,191,255,0.4);border-radius:24px;padding:40px 30px;display:flex;flex-direction:column;align-items:center;flex-shrink:0}
.kamusai-llm-input-box,.kamusai-llm-output-box{width:260px}
.kamusai-llm-model-box{background:rgba(122,229,130,0.1);border-color:rgba(122,229,130,0.5);width:220px}
.icon-emoji{font-size:80px;display:block;margin-bottom:20px}
.label-text{font-size:48px;font-weight:900;color:#fff}
.kamusai-llm-arrow-right,.kamusai-llm-arrow-left{font-size:72px;color:#00BFFF;font-weight:900}
.kamusai-llm-highlight{margin-top:100px;font-size:80px;font-weight:900;color:#FFD77A;text-shadow:0 0 40px rgba(255,215,122,0.6);border-bottom:4px solid #FFD77A}
.kamusai-mini-flow{z-index:15;padding:12px;background:rgba(0,0,0,0.4);border-radius:12px;display:flex;align-items:center;gap:4px}
